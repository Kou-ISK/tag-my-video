...
映像の再生部は `MovieTransportView` を共通に使う。中央に再生、一段内側に小送り、外側に大送りまたはクリップ移動を置く。速度は小さな倍率メニュー、時刻は単色の等幅文字とし、同じ画面に速度プリセットを重複して並べない。背景・区切り・アクセントは既存semantic tokenを使う。Playlistでは映像上の半透明パネル、Paintとメイン画面では固定ツールバーに合成する。操作量は呼び出し元が決め、秒送りをフレーム送りと表示しない。
...
[Sportscode公式マニュアルのMovie Controller](https://static.hudl.com/craft/SportsCodeManual-2.pdf)の矩形ジョグ部を参照し、左右のリブ、中央の再生、内外の送り領域を共通Viewへまとめる。メイン映像・Playlist・Paintで共用する。フレーム数が保証されない操作は秒送りのラベルを維持する。Playlistの描画目印は単なる色付きの点でなく、クリック可能なペンアイコンと時刻付きアクセシブル名で表す。
